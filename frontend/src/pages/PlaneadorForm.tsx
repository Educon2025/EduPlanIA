...
export default function PlaneadorForm({ showHistorialInitially = false }: { showHistorialInitially?: boolean }) {
  ...
  async function save() {
    ...
    try {
      ...
      setStatus(`✅ Planeador guardado con id ${data.id}`);
      alert("✅ Planeador guardado correctamente");
      if (showHistorial) {
        fetchHistorial();
      }
    } catch {
      setStatus("❌ Error al guardar planeador");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function generateWithAI() {
    ...
    // Construir descripción según tipo de planificación
    let descripcion = "";
    ...
    try {
      ...
      setStatus("✅ Planeador generado con IA");
    } catch (e: any) {
  ...
  async function deletePlaneador(id: number) {
    if (!confirm("¿Seguro que deseas eliminar este planeador?")) return;
    try {
      await api.delete(`/planeadores/${id}`);
      setHistorial((prev) => prev.filter((p) => p.id !== id));
      alert("🗑️ Planeador eliminado correctamente");
    } catch {
      alert("❌ Error al eliminar el planeador");
    }
  }
  ...
  function verPlaneador(planeador: any) {
    ...
    setShowHistorial(false);
    setStatus(`📄 Visualizando planeador ID: ${planeador.id}`);
  }
  ...
  function exportPDF(content: any, asignatura: string, grado: string, periodo: string, id?: number) {
    ...
    contenidoPDF += `Duración: ${contenidoReal.duracion || "10 semanas"}\n\n`;
    
    if (contenidoReal.estandares) {
      contenidoPDF += `ESTÁNDARES (MEN):\n`;
    ...
    if (contenidoReal.dba) {
      contenidoPDF += `DERECHOS BÁSICOS DE APRENDIZAJE (DBA):\n`;
    ...
    if (contenidoReal.estrategiasMetodologicas || contenidoReal.estrategias) {
      contenidoPDF += `ESTRATEGIAS METODOLÓGICAS:\n`;
  ...
  function exportWord(content: any, asignatura: string, grado: string, periodo: string, id?: number) {
    ...
    let htmlContent = `
      ...
      <body>
        ...
        <div class="info">
          ...
          <p><strong>Duración:</strong> ${contenidoReal.duracion || "10 semanas"}</p>
        </div>
    `;
    
    if (contenidoReal.estandares) {
      htmlContent += `<div class="section"><h2>ESTÁNDARES (MEN)</h2><ul>`;
      const est = Array.isArray(contenidoReal.estandares) ? contenidoReal.estandares : [contenidoReal.estandares];
    ...
    if (contenidoReal.dba) {
      htmlContent += `<div class="section"><h2>DERECHOS BÁSICOS DE APRENDIZAJE (DBA)</h2><ul>`;
      const dba = Array.isArray(contenidoReal.dba) ? contenidoReal.dba : [contenidoReal.dba];
    ...
    if (contenidoReal.estrategiasMetodologicas || contenidoReal.estrategias) {
      htmlContent += `<div class="section"><h2>ESTRATEGIAS METODOLÓGICAS</h2><ul>`;
      const estrategias = contenidoReal.estrategiasMetodologicas || contenidoReal.estrategias;
    ...
    <div className="space-y-3">
      ...
      {!showHistorialInitially && (
        <div className="flex justify-end mb-2">
          ...
          >
            📂 {showHistorial ? "Cerrar historial" : "Historial de planeadores"}
          </button>
        </div>
      )}
      ...
      {!preview && !showHistorial && (
        <>
          ...
          <textarea
            ...
            placeholder="Tema(s) a planificar (ej: Operaciones con números naturales, Resolución de problemas)"
            rows={3}
          />
      ...
      {showHistorial && (
        <div className="w-full bg-white shadow rounded-lg p-6 mb-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-gray-800">📚 Historial de Planeadores</h3>
            {!showHistorialInitially && (
              <button 
          ...
          {loadingHistorial ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <p className="text-lg text-gray-600 mb-2">📭 No hay planeadores guardados</p>
              <p className="text-sm text-gray-500">Crea tu primer planeador para verlo aquí</p>
            </div>
          ) : (
      ...
      {preview && !showHistorial && (
        <div className="mt-6 space-y-4">
          ...
          <section className="rounded-lg border border-gray-200 p-4 bg-white">
            <p className="font-semibold text-gray-800 mb-2">📋 Estándares (MEN)</p>
            <BulletList items={preview.estandares} />
          </section>

          <section className="rounded-lg border border-gray-200 p-4 bg-white">
            <p className="font-semibold text-gray-800 mb-2">🎯 Derechos Básicos de Aprendizaje (DBA)</p>
          ...
          {preview.contenidos && (
            <section className="rounded-lg border border-gray-200 p-4 bg-white">
              <p className="font-semibold text-gray-800 mb-2">📚 Contenidos</p>
          ...
          <section className="rounded-lg border border-gray-200 p-4 bg-white">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <CardSub title="Desarrollo">
                <BulletList items={preview.actividades?.desarrollo} />
              </CardSub>
              <CardSub title="Cierre">
<BulletList items={preview.actividades?.cierre} />
              </CardSub>
            </div>
          </section>

          <section className="rounded-lg border border-gray-200 p-4 bg-white">
            <p className="font-semibold text-gray-800 mb-2">🔧 Estrategias metodológicas</p>
            <BulletList items={preview.estrategiasMetodologicas ?? preview.estrategias} />
          </section>
          ...
          <section className="rounded-lg border border-gray-200 p-4 bg-white">
            ...
            <div className="space-y-2">
              ...
              {preview.evaluacion?.tipos && (
                <div>
                  <p className="text-sm font-medium text-gray-700">Tipos:</p>
                  <BulletList items={preview.evaluacion.tipos} />
                </div>
              )}
            </div>
          </section>
...
function BulletList({ items }: { items?: any }) {
  if (!items) return <p className="text-sm text-gray-600">Sin información</p>;
    ...
    <div className="rounded-lg border-2 border-gray-200 p-5 bg-white hover:shadow-lg transition-all duration-200 hover:border-blue-400">
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-500">
          📅 Periodo: {item.periodo} • 🆔 ID: {item.id}
        </p>
        {item.tema && (
